...
- `analiza_parametryczna.ipynb (Parametric AFT Models)` --references--> `TODO: Add Probability Plot for Log Survival Function`  [INFERRED]
  CLAUDE.md → notatak.txt
- `ACT_projekt_nieparam.sas (SAS PROC LIFETEST)` --rationale_for--> `Rationale: SAS Code Is Fuller/More Complete Reference`  [INFERRED]
  CLAUDE.md → notatak.txt
- `model_semiparametryczny.ipynb (Cox PH Semiparametric)` --references--> `TODO: Check Proportionality in Semiparametric Model`  [EXTRACTED]
  CLAUDE.md → notatak.txt
- `ACT_projekt_nieparam.sas (SAS PROC LIFETEST)` --references--> `TODO: Port SAS Code to Python`  [EXTRACTED]
  CLAUDE.md → notatak.txt
- `Cox Proportional Hazards Model` --references--> `TODO: Check Proportionality in Semiparametric Model`  [INFERRED]
  CLAUDE.md → notatak.txt
...
- **Thin community `Community 16`** (1 nodes): `Time-Varying Covariates (Cechy zmienne)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 17`** (1 nodes): `Secondary Covariates (Cechy wtórne) - unit differentiation`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 18`** (1 nodes): `Cross-Sectional Study (Badanie przekrojowe)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 19`** (1 nodes): `Panel Study (Badanie panelowe)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Nonparametric Models (Modele nieparametryczne)` connect `Community 1` to `Community 9`, `Community 5`?**
  _High betweenness centrality (0.038) - this node is a cross-community bridge._
- **Why does `Life Tables (Tablice trwania życia)` connect `Community 5` to `Community 1`?**
